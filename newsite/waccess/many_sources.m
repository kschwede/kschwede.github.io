...
sources.location = ...
 [ 7  2  6
  13 11 13.5 ]*la0; 
% use same excitation for all sources
sources.signal=cell(3,1);
sources.signal{1} = @(t) (t-t0).*exp(-pi^2*(dom.f0)^2*(t-t0).^2);
sources.signal{2} = @(t) (t-t0).*exp(-pi^2*(dom.f0)^2*(t-t0).^2);
sources.signal{3} = @(t) (t-t0).*exp(-pi^2*(dom.f0)^2*(t-t0).^2);
